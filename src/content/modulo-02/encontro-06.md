...
> **Dica:** As relações presentes neste diagrama são: 1× **Composição** `◆────→`, 1× **Agregação** `◇────→` e 2× **Associação** `────→`. Para cada par de classes, escolha qual tipo se aplica e escreva o rótulo da relação.
